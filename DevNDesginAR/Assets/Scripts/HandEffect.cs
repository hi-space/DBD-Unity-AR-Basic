...
{ // 3D joint skeleton
    ...
    void OnHumanBodiesChanged(ARHumanBodiesChangedEventArgs eventArgs)
    {
        foreach (ARHumanBody humanBody in eventArgs.updated)
        {
            ...
            {
                if (joint.index == (int)JointIndices3D.LeftHand || joint.index == (int)JointIndices3D.RightHand)
                {
                    GameObject obj;
                    if (!vertexObjects.TryGetValue(joint.index, out obj))
                    {
                        obj = Instantiate(handPrefab);
                    ...
                    if (joint.tracked)
                    {
                        ...
                        obj.transform.localScale = joint.anchorScale * 0.1f;
                        obj.SetActive(true);
                    }
                    else
                    {
